...
public class ScoreDao {
	...
	public int insert (Score score) throws Exception{
		...
		try {
			...
			pstmt= connection.prepareStatement(
					"insert into score(sname, korea, math, eng, total, avg, isFail) values(?,?,?,?,?,?,?)");// Statement 를 생성하겠다 
			pstmt.setString(1, score.getSname());
			pstmt.setInt(2, score.getKorea());
			pstmt.setInt(3, score.getMath());
			pstmt.setInt(4, score.getEng());
			pstmt.setInt(5, score.getTotal());
			pstmt.setFloat(6, score.getAvg());
			pstmt.setString(7, score.getIsFail());
	...
	public Score selectOne(int sno) throws Exception{ // update 서블릿의 doGet으로부터 가져온 코드 
		...
		try {
			...
			stmt= connection.createStatement(); 
			Score score = null;
			rs =stmt.executeQuery("select sno,sname, korea, math, eng from score " + 
					 " where sno = " + sno);
			if(rs.next()){
				score =  new Score().setSno(rs.getInt("sno"))
								.setSname(rs.getString("sname"))
								.setKorea(rs.getInt("korea"))
								.setMath(rs.getInt("math"))
								.setEng(rs.getInt("eng"));
	...
	public int update(Score score) throws Exception {
		...
		try {
			...
			pstmt = conn.prepareStatement("update score set korea =? ,math =?, eng =?, total=?,avg=?, isFail=?" +
					" where sno=?"
					);
			...
			pstmt.setFloat(5, score.getAvg());
			pstmt.setString(6, score.getIsFail());
			pstmt.setInt(7, score.getSno());
