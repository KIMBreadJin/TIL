...
public class MemberDao {
		...
		try {
			...
			if (rs.next()) {
				member = new Member().setNo(rs.getInt("MNO")).setName(rs.getString("MNAME")).setEmail(rs.getString("EMAIL"))
						 .setCreatedDate(rs.getDate("CRE_DATE")).setPassword(rs.getString("PWD"));
			}
			return member;
		}catch(Exception e) {
			throw e;
	...
	public int update(Member member) throws Exception {
		...
		try {
			...
			pstmt = connection.prepareStatement("update members set email =? ,mname=?, mod_date=now() " +
					" where mno  =?"
					);
			pstmt.setString(1, member.getEmail());
	...
	public Member exist(String email, String password) throws Exception{
		...
		ResultSet rs =null;
		try {
			pstmt=connection.prepareStatement("select email,mname from members  " + 
					 " where email = ? and pwd=? "); 
